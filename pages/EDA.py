...
# result = pd.read_csv('datasets_prep/result_df_with_dist.csv')
# result.u_g_dist = result['u_g_dist'].astype('int')
# result.u_g_dist = result.u_g_dist.apply(lambda x: x if x != 0 else 1)
result = ''
...
layout = html.Div([
    ...
    html.Div(id='tabs-content', children=generate_user_tab_content(users, result)),
...
def render_tabs_body(tab):
    if tab == 'users_tab':
        return generate_user_tab_content(users, result)
    elif tab == 'groups_tab':
        return generate_group_tab_content(groups_with_coords, dict_df)
    elif tab == 'attend_tab':
        return generate_attend_tab_content(result)
    else:
        return "404"
